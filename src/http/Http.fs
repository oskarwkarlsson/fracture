...
type ParserDelegate(onHeaders, requestBody, requestEnded) as p =
    ...
    interface IHttpParserHandler with
        ...
        member this.OnHeadersEnd(parser) = 
            ...
            onHeaders(p.requestHeaders, parser.ShouldKeepAlive)
...
type HttpParsedMessage =
    | Start of Tcp.Client
    | Headers of IPEndPoint * HttpRequestHeaders
    | Body of IPEndPoint * ArraySegment<byte> list

type AgentParserDelegate(agent: Agent<HttpParsedMessage>, client: Tcp.Client) as p =
    [<DefaultValue>] val mutable method' : string
    [<DefaultValue>] val mutable requestUri: string
    [<DefaultValue>] val mutable fragment : string
    [<DefaultValue>] val mutable queryString : string
    ...
    interface IHttpParserHandler with
        ...
        member this.OnHeadersEnd(parser) = 
            ...
            agent.Post(Headers(client.RemoteEndPoint, p.requestHeaders))
