...
type ParserDelegate(app, send: bool -> byte[] -> bool, ?onHeaders, ?requestBody, ?requestEnded) as p =
    ...
    let run app req keepAlive = async {
        ...
        for line : ArraySegment<_> in responseToBytes res do
            send keepAlive line.Array.[line.Offset..(line.Offset + line.Count - 1)] |> ignore
    }
