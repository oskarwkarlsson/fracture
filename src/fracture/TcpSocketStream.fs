...
module private Impl =
    let asyncRead buffer offset count (received: IEvent<byte[]*_>) =
        ...
        async {
            let! (data, _) = Async.AwaitEvent <| received
...
//                Buffer.BlockCopy(args.Buffer, args.Offset, buffer, offset, bytesRead)
    ...
    let asyncWrite (buffer: byte[]) offset count (send: byte[] * int * int -> unit) =
        if buffer = null then
...
type TcpSocketStream internal (socket: Socket, endPoint: IPEndPoint, received: IEvent<byte[] * SocketDescriptor>, sent: IEvent<byte[] * SocketDescriptor>, send) as x =
    ...
    // TODO: Expose Received and Sent events?
    ...
    member x.AsyncRead(buffer: byte[], offset, count) =
        received |> Impl.asyncRead buffer offset count
    ...
    member x.AsyncWrite(buffer: byte[], offset, count) =
        send !keepAlive |> Impl.asyncWrite buffer offset count
